<template>
  <div class="delete-area">
    <div class="delete-btn" @click="toggleDeleteModal = !toggleDeleteModal">Delete Project</div>
    <!-- Modal -->
    <div v-if="toggleDeleteModal"
          class="fixed inset-0 flex justify-center items-center z-50 ">
        <div
        class="delete-modal relative mx-auto max-w-4xl">
          <div class="bg-white w-full shadow-2xl max-w-2xl rounded">
            <div class="modal-title">Are you absolutely sure?</div>
            <p class="modal-type">Please type <span class="font-semibold italic">delete</span> to confirm.</p>
            <input class="modal-input"  v-model="deleteText"><br>
            <button class="modal-btn" @click="deleteProject1">
              Delete
            </button> 
    ...
    <div v-if="toggleDeleteModal" class="absolute z-40 inset-0 opacity-25 bg-black"></div>

    <div v-if="toggleConfirmModal"
          class="fixed overflow-x-hidden overflow-y-auto inset-0 flex justify-center items-center z-50 ">
        <div class="confirm-modal relative relative mx-auto max-w-4xl">
            <div class="loader"></div>
        </div>
      </div>
      <div v-if="toggleConfirmModal" class="absolute z-40 inset-0 opacity-25 bg-black"></div>
  </div>
</template>
